...
fn main() {
    ...
    // Coordinates are DrawBot's: origin bottom-left, y-up, and text() puts
    // the BASELINE of the first line at y.

    // Example 1: Simple text using custom font
    ctx.fill(Color::rgb(20, 20, 20));
    ctx.font_size(96.0);
    ctx.text("Hello DesignBot", 100.0, 830.0);

    // Example 2: Red Text
    ctx.fill(Color::rgb(200, 50, 50));
    ctx.font_size(128.0);
    ctx.text("Big Red Font", 95.0, 680.0);

    // Example 3: Blue Text
    ctx.fill(Color::rgb(50, 100, 200));
    ctx.font_size(64.0);
    ctx.text("Blue Font", 100.0, 550.0);
    ...
    // text_box anchors the box at its bottom-left corner; text fills from
    // the top of the box down.
    ctx.text_box(text_content, 100.0, 50.0, 800.0, 400.0);

    // Draw a rectangle to show the text box bounds
    ctx.no_fill();
    ctx.stroke(Color::rgb(50, 50, 50));
    ctx.stroke_width(4.0);
    ctx.rect(100.0, 50.0, 800.0, 400.0);
