//! A foundry specimen, the reusable way: a theme + a canonical format + plain
//! DrawBot `text()` calls. This is the template a font repo copies into its
//! own `documentation/` as a per-image script. Everything reusable — the
//! theme, the OKLCH palette, the size — lives in designbot; only the content
//! and the explicit layout numbers live here.
//!
//!     cargo run --example specimen        # writes specimen.png
//!
//! (As a font-repo script it would run via: designbot --render specimen.rs
//!  --output specimen.png --social)

use designbot::prelude::*;

fn main() {
    // --- design system + canvas ---
    let t = Theme::dark(); // try Theme::light() / Theme::black()
    let f = Format::Square; // Portrait / Landscape / Vertical
    let (w, h) = (f.w(), f.h());
    let m = f.margin();

    let mut ctx = Canvas::new(w, h);
    let mut r = Renderer::new(w as u32, h as u32);
    r.load_font("/Users/eli/GH/repos/virtua-grotesk/fonts/ttf/VirtuaGrotesk-Regular.ttf")
        .expect("load Virtua Grotesk");
    r.load_font("/Users/eli/GH/repos/google-fonts/ofl/geistmono/GeistMono[wght].ttf")
        .expect("load Geist Mono");

    ctx.background(t.ground);

    // --- corner furniture (mono) — edit the strings freely ---
    ctx.font("Geist Mono").font_size(32.0).fill(t.furniture);
    ctx.text_align(TextAlign::Left).text("Font.Garden/virtua", m, h - m);
    ctx.text_align(TextAlign::Right).text("Open Font License OFL v1.1", w - m, h - m);
    ctx.text_align(TextAlign::Left).text("Virtua Grotesk Regular v0.1", m, m);
    ctx.text_align(TextAlign::Right).text("github.com/eliheuer/virtua-grotesk", w - m, m);

    // --- hairline rules ---
    ctx.no_fill().stroke(t.rule).stroke_width(2.0);
    ctx.line(m, h - m - 56.0, w - m, h - m - 56.0);
    ctx.line(m, m + 64.0, w - m, m + 64.0);

    // --- specimen rows — explicit numbers, DrawBot-style ---
    ctx.fill(t.ink).font("Virtua Grotesk").text_align(TextAlign::Left);
    let (size, lead) = (210.0, 214.0);
    let mut y = h - m - 200.0;
    for row in [
        "ABCDEFGHIJ", "KLMNOPQR", "STUVWXYZ", "0123456789", "abcdefghij", "klmnopqr", "stuvwxyz",
    ] {
        ctx.font_size(size).text(row, m, y);
        y -= lead;
    }

    r.render_to_png(&ctx, "specimen.png").expect("render");
    println!("wrote specimen.png");
}
